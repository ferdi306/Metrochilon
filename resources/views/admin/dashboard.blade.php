...
<head>
    ...
    <style>
        ...
        *{ box-sizing:border-box; }
        ...
        .content{ display:grid; grid-template-columns: 1fr; gap:28px; max-width:1400px; width:100%; }
        .mobile-menu-btn{ 
            display:none; 
            background:var(--primary); 
            color:#fff; 
        ...
        .badge-danger{ background:#fee2e2; color:#991b1b; padding:4px 8px; border-radius:6px; font-size:11px; font-weight:600; }
        .btn-sm{ padding:6px 12px; border-radius:8px; font-size:12px; font-weight:600; text-decoration:none; display:inline-block; transition: all .3s ease; }
        .btn-primary{ background:linear-gradient(135deg, var(--primary), var(--accent)); color:#fff; }
        .btn-primary:hover{ transform: translateY(-2px); box-shadow: 0 4px 12px rgba(0,102,255,.3); }
            ...
            <main class="content">
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap:24px; margin-bottom:32px;">
                    <div class="card pad" style="text-align:center; animation: fadeInUp 0.6s ease-out 0.1s backwards;">
                        <div style="font-size:32px; font-weight:700; color:var(--primary); margin-bottom:8px;" class="counter" data-target="{{ $totalEmployees }}">0</div>
                        <div style="color:var(--muted); font-size:14px;">Total Karyawan</div>
                    </div>
                    <div class="card pad" style="text-align:center; animation: fadeInUp 0.6s ease-out 0.2s backwards;">
                        <div style="font-size:32px; font-weight:700; color:var(--primary); margin-bottom:8px;" class="counter" data-target="{{ $totalAttendances }}">0</div>
                        <div style="color:var(--muted); font-size:14px;">Absensi Hari Ini</div>
                    </div>
                    <div class="card pad" style="text-align:center; animation: fadeInUp 0.6s ease-out 0.3s backwards;">
                        <div style="font-size:32px; font-weight:700; color:var(--primary); margin-bottom:8px;" class="counter" data-target="{{ $activeLocations }}">0</div>
                        <div style="color:var(--muted); font-size:14px;">Lokasi Aktif</div>
                    </div>
                </div>
                <style>
                    @@keyframes fadeInUp {
                ...
                <div class="card pad">
                    <div class="hero">
                        <div>
                            ...
                            <small>5 absensi terbaru</small>
                        </div>
                        <a href="{{ route('admin.attendances.page') }}" style="color:var(--primary); text-decoration:none; font-size:14px; font-weight:600;">Lihat Semua →</a>
                    </div>
                    <div class="pad">
                        <table>
                </div>
                ...
                <div class="card pad">
                    <div class="hero">
                        <div>
                            <h3>Analytics Absensi (Bulan Ini)</h3>
                            <small>Hadir / Izin / Sakit per hari</small>
                        </div>
                        <div style="display:flex; gap:8px; align-items:center;">
                            <select id="analyticsMonth" style="padding:8px; border-radius:8px; border:1px solid #e5e7eb;">
                                @for($m=1;$m<=12;$m++)
                                    @php $val = now()->format('Y') . '-' . sprintf('%02d', $m); @endphp
                                    <option value="{{ $val }}" {{ $val === now()->format('Y-m') ? 'selected' : '' }}>{{ $val }}</option>
                                @endfor
                            </select>
                        </div>
                    </div>
                    <div class="pad">
                        ...
                        <script>
                            async function loadAttendanceChart(month) {
                                const res = await fetch(`{{ url('/admin/api/analytics/attendance/daily') }}?month=${month}`);
                                ...
                                window.attChart = new Chart(ctx, {
                                    type: 'bar',
                                    data: {
                                        labels: data.labels.map(l => l.slice(8)), // day of month
                                        datasets: [
                                            { label: 'Hadir', data: data.present, backgroundColor: '#10b981' },
                                            { label: 'Izin', data: data.izin, backgroundColor: '#f59e0b' },
                                            { label: 'Sakit', data: data.sakit, backgroundColor: '#ef4444' },
                                        ]
                                    },
                            ...
                            document.getElementById('analyticsMonth').addEventListener('change', (e) => loadAttendanceChart(e.target.value));
                            document.addEventListener('DOMContentLoaded', () => loadAttendanceChart(document.getElementById('analyticsMonth').value));
                        </script>
                    </div>
                </div>
                            <thead>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($attendance->photo_path)
                                            <a href="{{ asset('storage/'.$attendance->photo_path) }}" target="_blank" style="color:var(--primary); text-decoration:none; font-weight:600;">Lihat</a>
                                        @else
                                ...
                                @empty
                                <tr>
                                    <td colspan="4" style="text-align:center; padding:24px; color:#9ca3af;">Belum ada data absensi</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
